...
async def upload_pdfs(files: List[UploadFile] = File(...), tmp_paths=None):
    """
    支持多文件上传的PDF处理接口
    - 一次可上传多个PDF文件
    - 自动向量化并加入FAISS知识库
    ...
    for f in files:
        if not f.filename.lower().endswith('.pdf'):
            raise HTTPException(400, f"文件 {f.filename} 不是PDF格式")
    ...
    try:
        ...
        for f in files:
            with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
                ...
                temp_paths.append({"name": tmp.name, "filename": f.filename})

        # 构造伪文件对象（兼容 rag_demo_pro.process_multiple_pdfs）
        file_objs = [type("obj", (object,), {"name": fp["name"]}) for fp in temp_paths]
        ...
        # result_text 通常返回 ("处理成功，共 X 个文本块", [文件列表])
        msg = result_text[0] if isinstance(result_text, tuple) else str(result_text)

        # 匹配分块数量
        chunk_match = re.findall(r'(\d+) 个文本块', msg)
        if chunk_match:
            total_chunks = sum(map(int, chunk_match))
        ...
        return {
            "status": "success",
            "message": f"成功处理 {len(files)} 个文件，共 {total_chunks} 个文本块。",
            "file_info": {
        ...
        logger.error(f"多文件上传处理失败: {str(e)}")
